...
local function DisplayMedals(SCROLL, DATA, P, PAGE, ADMIN)
    ...
    for k, v in ipairs(DATA) do
        ...
        if DIS_PMEDAL then
            PRIMARY = vgui.Create("RDV_LIBRARY_TextButton", BUT)
            PRIMARY:SetText("")
            PRIMARY:Dock(RIGHT)
            ...
            PRIMARY.PaintOver = function(s, w, h)
                ...
                if PRIM and ( PRIM == v.UID ) then
                    draw.SimpleText("Primary", "RDV_LIB_FRAME_TITLE", w * 0.5, h * 0.5, COL, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
                else
                    draw.SimpleText("Make Primary", "RDV_LIB_FRAME_TITLE", w * 0.5, h * 0.5, COL, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
                end
            end
        ...
        if CFG.ICON then
            ICON = vgui.Create("RDV_LIBRARY_IMGUR", BUT)
            ICON:SetParent(BUT)
            ICON:Dock(LEFT)
            ICON:SetImgurID(CFG.ICON)
            ICON:SetImageSize(0.75)
        end
    end

    for i = 1, 2 do
        local NEXT = SCROLL:Add("RDV_LIBRARY_TextButton")
        NEXT:SetText("")
...
local function DisplayPlayers(SCROLL)
    ...
    for k, v in ipairs(player.GetAll()) do
        ...
        local BUT = SCROLL:Add("RDV_LIBRARY_TextButton")
        BUT:SetSize(w, h * 0.125)
        BUT:SetText(v:Name())
        BUT:Dock(TOP)
        BUT:DockMargin(w * 0.01, 0, w * 0.01, h * 0.01)
        ...
        BUT.DoClick = function(self)
            ...
            MenuButtonOptions:AddOption(RDV.LIBRARY.GetLang(nil, "MED_giveMedal"), function()                
                ...
                for k, v in pairs(RDV.MEDALS.LIST) do
                    ...
                    BUT_GIVE = vgui.Create("RDV_LIBRARY_TextButton", BUT)
                    BUT_GIVE:SetText("")
                    BUT_GIVE:Dock(RIGHT)
...
net.Receive("RDV_MEDALS_MENU", function()
    ...
    local P = vgui.Create("RDV_LIBRARY_FRAME")
    P:SetSize(ScrW() * 0.4, ScrH() * 0.6)
    P:Center()
    P:MakePopup()
    P:SetTitle(RDV.LIBRARY.GetLang(nil, "MED_titleName"))

    local S = vgui.Create("RDV_LIBRARY_SIDEBAR", P)
